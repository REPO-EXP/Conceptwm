...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--wm_pretrained_model_name_or_path",
        type=str,
        default=None,
        required=True,
        help="Path to pretrained model or model identifier from huggingface.co/models.",
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        image_files = [f for f in os.listdir(instance_data_root) if f.endswith('.png') or f.endswith('.jpg')]
        image_files.sort(key=lambda x: (int(x.split('_')[0]), int(x.split('_')[1].split('.')[0])))
        image_files_full_paths = [os.path.join(instance_data_root, image_file) for image_file in image_files]
        self.instance_images_path = image_files_full_paths
        
        image_files = [f for f in os.listdir(wm_instance_data_root) if f.endswith('.png') or f.endswith('.jpg')]
        image_files.sort(key=lambda x: (int(x.split('_')[0]), int(x.split('_')[1].split('.')[0])))
        image_files_full_paths = [os.path.join(wm_instance_data_root, image_file) for image_file in image_files]
        self.wm_instance_images_path = image_files_full_paths
    ...
    def __getitem__(self, index):
        ...
        wm_instance_image = Image.open(self.wm_instance_images_path[index % self.num_instance_images])
        
        if not instance_image.mode == "RGB":
            instance_image = instance_image.convert("RGB")
        if not wm_instance_image.mode == "RGB":
            wm_instance_image = wm_instance_image.convert("RGB")
            
        example["instance_images"] = self.image_transforms(instance_image)
        example["wm_instance_images"] = self.image_transforms(wm_instance_image)
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    wm_pixel_values=[example["wm_instance_images"] for example in examples]
    ...
    wm_pixel_values = torch.stack(wm_pixel_values)
    wm_pixel_values = wm_pixel_values.to(memory_format=torch.contiguous_format).float()
    ...
    batch = {
        ...
        "wm_pixel_values": wm_pixel_values,
    }
    return batch
...
class LatentsDataset(Dataset):
    def __init__(self, latents_cache, text_encoder_cache,wm_latents_cache):
        self.latents_cache = latents_cache
        self.text_encoder_cache = text_encoder_cache
        self.wm_latents_cache=wm_latents_cache
    def __len__(self):
        return len(self.latents_cache)

    def __getitem__(self, index):
        return self.latents_cache[index],self.text_encoder_cache[index],self.wm_latents_cache[index]
    
def weight_modulation(
    args,
    models,
    noise_scheduler,
    batch,
    ...
    perturbed_images = (batch[0][0].sample())[0].unsqueeze(0).detach().clone().float()
    perturbed_images.requires_grad_(True)
    import torch.optim as optim
    optimizer = optim.Adam([perturbed_images], lr=1e-7)
    for step in range(num_steps):
        
        latents=perturbed_images
        ...
        encoder_hidden_states = batch[0][1][0].unsqueeze(0)
        ...
        original_images=(batch[0][2].sample())[0].unsqueeze(0).detach().clone().to(weight_dtype)
        wm_tensor=batch[0][0].sample()[0].unsqueeze(0).detach().clone().to(weight_dtype)
...
unet = UNet2DConditionModel.from_pretrained(
    args.wm_pretrained_model_name_or_path, subfolder="unet", revision=args.revision
)
...
train_dataset = DreamBoothDataset(
    ...
    wm_instance_data_root=args.wm_instance_data_dir,
...
for batch in tqdm(train_dataloader, desc="Caching latents"):
    with torch.no_grad():
        ...
        batch["wm_pixel_values"] = batch["wm_pixel_values"].to(accelerator.device, non_blocking=True, dtype=weight_dtype)
        batch["input_ids"] = batch["input_ids"].to(accelerator.device, non_blocking=True)
        latents_cache.append(vae.encode(batch["pixel_values"]).latent_dist)
        wm_latents_cache.append(vae.encode(batch["wm_pixel_values"]).latent_dist)
        if args.train_text_encoder:
            text_encoder_cache.append(batch["input_ids"])
        else:
            text_encoder_cache.append(text_encoder(batch["input_ids"])[0])
train_dataset = LatentsDataset(latents_cache, text_encoder_cache,wm_latents_cache)
...
for epoch in range(first_epoch, args.num_train_epochs):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        with accelerator.accumulate(unet):
            ...
            latent_ori = batch[0][2]
            
            latents = latent_dist.sample()
            latents_batch = latents * scaling_factor
            latents_ori = latent_ori.sample()
            latents_ori = latents_ori.repeat(2, 1, 1, 1) * scaling_factor
            
            latents = weight_modulation(
                args,
                [unet],
                noise_scheduler,
                batch,
            ...
            latents=torch.stack((latents[0],latents_batch[0]))
